...
async function migratePersons() {
    console.log('--- Starte Migration für die Personenanzahl (persons) ---');
    let pool;
    try {
        ...
        console.log(`${plans.length} Pläne werden auf fehlende Personenanzahl geprüft.`);
        ...
        for (const plan of plans) {
            ...
            // 3. Prüfe, ob 'persons' bereits existiert.
            if (!settings.hasOwnProperty('persons')) {
                // 4. Wenn es fehlt, füge es mit dem Standardwert '2' hinzu
                settings.persons = 2;
                ...
                console.log(`  -> Plan-ID ${plan.id} aktualisiert: persons auf '2' gesetzt.`);
                totalPlansUpdated++;
            }
        }
