...
// Helper to get unique recipes from the entire archive, now including their image URLs
const getUniqueRecipes = (archive: ArchiveEntry[]): PlannerRecipe[] => {
    const uniqueRecipes = new Map<string, PlannerRecipe>();
    archive.forEach(plan => {
        (plan.recipes || []).forEach(recipe => {
            if (!uniqueRecipes.has(recipe.title)) { // Use title to deduplicate
                const imageUrl = (plan.imageUrls && plan.imageUrls[recipe.day]) ? plan.imageUrls[recipe.day] : undefined;
...
const PlannerComponent: React.FC<PlannerComponentProps> = ({ archive, onPlanSaved }) => {
    ...
    // Planner states
    const [weeklySlots, setWeeklySlots] = useState<(PlannerRecipe & { uniqueId: string })[]>(Array(7).fill(null));
    ...
    const handleSavePlan = async () => {
        ...
        const customPlanPayload = {
            ...
            // imageUrls werden nicht mehr gesendet
        };
    ...
    const handleSelectRecipeForDay = (recipe: PlannerRecipe) => { if (modalDayIndex !== null) { const newSlots = [...weeklySlots]; newSlots[modalDayIndex] = { ...recipe, uniqueId: `${modalDayIndex}-${Date.now()}` }; setWeeklySlots(newSlots); } setIsModalOpen(false); setModalDayIndex(null); };
            ...
            <FilterControls {...filterProps} />
             <div className="space-y-3 flex-grow">
                {filteredRecipes.length > 0 ? filteredRecipes.map((recipe, index) => (
                    <div key={`${recipe.sourcePlanId}-${index}`} onClick={() => handleSelectRecipeForDay(recipe)} className="bg-white p-3 rounded-md shadow hover:shadow-md cursor-pointer border border-slate-200 active:bg-emerald-50 flex items-center gap-4">
                        {recipe.imageUrl && (
                            <img src={recipe.imageUrl} alt={recipe.title} className="w-16 h-16 rounded-md object-cover flex-shrink-0" />
                        )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                            {filteredRecipes.length > 0 ? filteredRecipes.map((recipe, index) => (
                                <div key={`${recipe.sourcePlanId}-${index}`} draggable onDragStart={(e) => handleDragStart(e, recipe)} className="bg-white p-3 rounded-md shadow hover:shadow-md cursor-grab active:cursor-grabbing border border-slate-200 flex items-center gap-4">
                                     {recipe.imageUrl && (
                                        <img src={recipe.imageUrl} alt={recipe.title} className="w-12 h-12 rounded-md object-cover flex-shrink-0 pointer-events-none" />
                                    )}
                                    <div className="flex-grow">
                                        <p className="font-semibold text-slate-700">{recipe.title}</p>
                                        <p className="text-xs text-slate-400">{recipe.totalCalories} kcal &bull; {dietPreferenceLabels[recipe.sourcePlanSettings.dietaryPreference]}</p>
