...
const GeneratedRecipeImage: React.FC<GeneratedRecipeImageProps> = ({ recipeTitle, imageUrl, isLoading, error, onGenerate }) => {
  // If an image URL is present, render the image with its natural aspect ratio.
  if (imageUrl) {
    return (
      <div className="relative bg-slate-200">
        <img src={imageUrl} alt={`Ein Bild von ${recipeTitle}`} className="w-full h-auto" />
      </div>
    );
  }

  // If there's no image (yet), render a placeholder with a fixed aspect ratio for a consistent layout.
  return (
