...
const RecipeCard: React.FC<{ recipe: Recipe; onSelect: (recipe: Recipe) => void }> = ({ recipe, onSelect }) => {
    return (
        <div 
            className="bg-white dark:bg-slate-800 rounded-xl shadow-md overflow-hidden transition-shadow hover:shadow-lg flex flex-col cursor-pointer"
        ...
        >
            <div className="aspect-video bg-slate-100 dark:bg-slate-700 flex items-center justify-center">
                {recipe.thumbnail_url || recipe.image_url ? (
                    ...
                    <svg className="w-full h-full text-slate-300 dark:text-slate-600 p-10" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.409a2.25 2.25 0 013.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z" />
                    </svg>
                )}
            </div>
            <div className="p-5 flex flex-col flex-grow">
                <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400 tracking-wide uppercase">{MealCategoryLabels[recipe.category]}</p>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mt-1 flex-grow leading-tight">{recipe.title}</h3>
                <div className="mt-3 flex flex-wrap gap-2">
                    <span className="text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-1 rounded-full">{recipe.totalCalories} kcal</span>
                    {recipe.dietaryPreference && <span className="text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2 py-1 rounded-full capitalize">{recipe.dietaryPreference === 'omnivore' ? 'Alles' : recipe.dietaryPreference}</span>}
                </div>
            </div>
        </div>
    );
...
const RecipeArchiveComponent: React.FC = () => {
    ...
    if (isLoading) {
        return <div className="text-center py-16 text-slate-600 dark:text-slate-300">Lade Rezepte...</div>;
    }
        ...
        <div className="space-y-0" ref={recipeArchiveContainerRef}>
            ...
            {filteredRecipes.length > 0 ? (
                ...
                <div className="text-center py-16 bg-white dark:bg-slate-800 rounded-lg shadow-md">
                    <h2 className="text-xl font-bold text-slate-600 dark:text-slate-300 mb-2">Keine Rezepte gefunden</h2>
                    <p className="text-slate-500 dark:text-slate-400">Für die aktuellen Filter wurden keine Rezepte gefunden. Versuchen Sie, die Filter anzupassen.</p>
                </div>
            )}
